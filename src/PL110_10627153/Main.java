...
class Global {
  ...
  static public ICEInputStream sc;
...
class CutToken {
  ...
  protected void IDUUDEFINED() throws Throwable {
    if ( mBuffer.get( mBuffer.size() - 1 ).GetType() == Global.s_T_ID ) {
      ...
      else if ( mBuffer.get( mBuffer.size() - 1 ).GetToken().equals( "cin" ) ||
                mBuffer.get( mBuffer.size() - 1 ).GetToken().equals( "cout" ) ||
                mBuffer.get( mBuffer.size() - 1 ).GetToken().equals( "ListAllVariables" ) ||
                mBuffer.get( mBuffer.size() - 1 ).GetToken().equals( "ListAllFunctions" ) ||
                mBuffer.get( mBuffer.size() - 1 ).GetToken().equals( "ListVariable" ) ||
                mBuffer.get( mBuffer.size() - 1 ).GetToken().equals( "ListFunction" ) ||
                mBuffer.get( mBuffer.size() - 1 ).GetToken().equals( "Done" ) &&
      ...
      if ( mBuffer.size() > 1 && mBuffer.get( mBuffer.size() - 2 ).GetType() == Global.s_T_ID ) {
        System.out.println( "Line " + mBuffer.get( mBuffer.size() - 1 ).Getline() + " : " +
                            "unexpected token : '"
                            + mBuffer.get( mBuffer.size() - 1 ).GetToken() + "'" );
        // System.out.print( "> " );
...
class Main {
  ...
  public static void main( String[] args ) throws Throwable {
    Global.sc = new ICEInputStream();
    CutToken cutToken = new CutToken();
    Global.G_OurCInitialize();
    String tt = Global.sc.ReadInputLine();
    if ( tt.contains( "2" ) )
      ; // System.exit( 0 );
